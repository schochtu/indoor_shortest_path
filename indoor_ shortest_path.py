...
def vert_search(start, goal, grid, vert_dir, dist, openList):
    x0 = start.x
    leng = len(openList)
    ...
    while True:
        ...
        if (grid.is_wall(grid.g[x1][start.y-1])) and not (grid.is_wall(grid.g[x2][start.y-1])):
            ...
            openList.append(grid.g[x2][start.y-1])
        ...
        if len(openList) > leng:
            openList.append(grid.g[x1][start.y])
            return openList
...
def hor_search(start, goal, grid, hor_dir, dist, openList):
    y0 = start.y
    leng = len(openList)
    ...
    while True:
        ...
        if (grid.is_wall(grid.g[start.x-1][y1])) and not (grid.is_wall(grid.g[start.x-1][y2])):
            ...
            openList.append(grid.g[start.x-1][y2])
        ...
        if len(openList) > leng:
            openList.append(grid.g[start.x][y1])
            return openList
...
def diag_search(start, goal, grid, hor_dir, vert_dir, dist, openSet):
    ...
    while True:
        ...
        if not (grid.in_bounds(x1, y1)):
            return [] #Off grid - done

        if (grid.is_wall(grid.g[x1][y1])):
            return [] #Is wall - done
        
        if ((y1 == goal.y) and (x1 == goal.x)):
            grid.g[x1][y1].cameFrom = grid.g[current.x][current.y]
            grid.g[x1][y1].gScore = dist + np.sqrt(2)
        ...
        if (grid.is_wall(grid.g[current.x][y1])) and not (grid.is_wall(grid.g[current.x][y2])):
            grid.g[x2][y2].cameFrom = grid.g[x1][y1]
        ...
        if len(openSet) == leng:
            sub_nodes = hor_search(grid.g[x1][y1], goal, grid, hor_dir, dist, [])
            hor_done = True

        if len(sub_nodes) > 0: 
            # Horizontal search ended with a jump point. 
            for sub in sub_nodes: 
                sub.cameFrom = grid.g[x1][y1]
                openSet.append(sub)
        ...
        if len(openSet) == leng: 
            sub_nodes = vert_search(grid.g[x1][y1], goal, grid, vert_dir, dist, [])
            vert_done = True 
        ...
        if len(openSet) > leng:
            if not hor_done:
                grid.g[x1][y2].cameFrom = grid.g[x1][x1]
                openSet.append(grid.g[x1][y2]) 
              
            if not vert_done:
                grid.g[x2][y1].cameFrom = grid.g[x1][y1] 
                openSet.append(grid.g[x2][y1])
...
def jump_point_search(start, goal, grid, heuristic):
    ...
    while not (len(openSet) == 0):
        ...
        openSet.remove(current)
        if not origin:
            vert_search(current, goal, grid, 1, dist, openSet)
            vert_search(current, goal, grid, -1, dist, openSet)
            hor_search(current, goal, grid, 1, dist, openSet)
            hor_search(current, goal, grid, -1, dist, openSet)
            origin = True
        diag_search(current, goal, grid, -1, 1, dist, openSet)
        diag_search(current, goal, grid, 1, 1, dist, openSet)
        diag_search(current, goal, grid, 1, -1, dist, openSet)
        diag_search(current, goal, grid, -1, -1, dist, openSet)
...
def calc_jpoint(event):
    print("calc_jpoint")
    #result = jps.method(grid, grid.g[coords[0]][coords[1]], grid.g[coords[2]][coords[3]])
...
jumppointx = plt.axes([0.65, 0.025, 0.12, 0.04])
...
astarx = plt.axes([0.5, 0.025, 0.12, 0.04])
